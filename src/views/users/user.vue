<template>
  <el-card class="box-card">
    ...
    <el-table class="table-users" :data="usersList" style="width: 100%">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          ...
          ></el-button>
          <!-- eidtUser(scope.row.id,scope.row.username,scope.row.email,scope.row.mobile,) -->
          <el-button
            type="primary"
            :plain="true"
            size="small"
          ...
          <el-button
            ...
            icon="el-icon-check"
            circle
            @click="showModifyRoleDialog(scope.row)"
          ></el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    >
      <!-- current_edituser_info current_edituser_username -->
      <el-form :model="user_info">
        <el-form-item label="姓名" :label-width="formLabelWidth">
          <el-input :value="user_info.username" disabled autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="邮箱" :label-width="formLabelWidth">
          <el-input v-model="user_info.email" autocomplete="off"></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    async saveUserRole(){
      ...
      let ret_user= await this.$http.put(`users/${this.user_info.id}/role`,{rid:this.currentRoleId})       
      //console.log(ret_user);
      
      this.dialogFormVisibleRole = false
    },
    async showModifyRoleDialog(user) {
      this.user_info = user
      //找到 userid, 找到 角色id , 找到角色列表,
      
      let tk = localStorage.getItem("mytoken");
      this.$http.defaults.headers.common[`Authorization`] = tk;
      let ret_user= await this.$http.get(`users/${this.user_info.id}`)
      this.currentRoleId = ret_user.data.data.rid
      //console.log(this.currentRoleId);

      let ret = await this.$http.get(`roles/${this.currentRoleId}`);
      console.log(ret);

      let ret_roles = await this.$http.get(`roles`);
      this.roles= ret_roles.data.data
      //console.log(this.roles); //roles [] {id,rolename,}
      let cur_role_id_have = this.roles.some((v,i)=>{
        return v.id == this.currentRoleId
      })
      if(!cur_role_id_have){
    ...
    async saveUser() {
      ...
      let allValid = this.user_info.email && this.user_info.mobile;
      if (!allValid) {
        alert("请完善信息");
        return;
      }
